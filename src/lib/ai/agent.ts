/**
 * Single-agent loop for Seeker Code.
 * Streams, parses tool blocks, executes, feeds results back.
 */
import { streamProfile, type ChatMessage, type CallProfile } from "./nvidiaClient";
...
export async function runAgent(opts: {
  ...
  const { settings, profile, system, ctx, callbacks, signal } = opts;
  ...
  const maxSteps = profile?.maxSteps ?? INTERNAL_DEFAULTS.maxAgentSteps;
