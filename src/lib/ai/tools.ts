...
function numbered(text: string, offset = 0, max = 1200): string {
  ...
  try {
    switch (call.tool) {
      case "read_file": {
        ...
        const end = Number(call.args.end) || Math.min(start + 799, text.split("\n").length);
        const slice = text.split("\n").slice(start - 1, end).join("\n");
        return { ...base, ok: true, mutating: false, path, output: numbered(slice, start - 1) || "(empty file)" };
      }
      case "list_dir": {
