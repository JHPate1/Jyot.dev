...
const IDB_KEY = "kode:memfs:v1";
const LEGACY_IDB_KEY = "nvide:memfs:v1";

export class MemoryFS implements VFS {
  id: BackendId = "memory";
  label = "In-memory (IndexedDB cached)";
  ...
  static async load(seed: Record<string, string>): Promise<MemoryFS> {
    try {
      const stored =
        ((await get(IDB_KEY)) as Record<string, string> | undefined) ||
        ((await get(LEGACY_IDB_KEY)) as Record<string, string> | undefined);
  ...
  async listTree(): Promise<VNode> {
    ...
    for (const [p, meta] of this.files) {
      ensureDir(dirname(p)).children!.push({
        path: p,
        name: basename(p),
        kind: "file",
  ...
  async stat(path: string) {
    const f = this.files.get(normalizePath(path));
    return f ? { size: f.content.length, mtime: f.mtime } : null;
  }
